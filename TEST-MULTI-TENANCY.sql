-- TEST MULTI-TENANCY SYSTEM - EVERYTHING SHOULD WORK NOW

-- 1. Verify table structure
SELECT 
    'vba_projects columns' as check,
    COUNT(*) as column_count
FROM information_schema.columns
WHERE table_schema = 'public' 
AND table_name = 'vba_projects';

-- 2. Test creating a VBA project with all proper fields
INSERT INTO vba_projects (
    name,
    project_name, 
    project_number,
    address,
    city,
    state,
    contractor,
    owner,
    status,
    organization_id,
    created_by
)
VALUES (
    'Test VBA Project - ' || NOW()::text,
    'Beta Testing Project',
    'VBA-2025-001',
    '123 Test Street',
    'Naples',
    'FL',
    'Test Contractor Inc',
    'Beta Testing Owner',
    'active',
    '11111111-1111-1111-1111-111111111111'::uuid,
    auth.uid()
)
RETURNING id, name, project_name, organization_id;

-- 3. Create multiple test projects to verify persistence
INSERT INTO projects (name, description, status, organization_id, created_by)
SELECT 
    'Project ' || generate_series,
    'Testing persistence #' || generate_series,
    'active',
    '11111111-1111-1111-1111-111111111111'::uuid,
    auth.uid()
FROM generate_series(1, 3);

-- 4. Create test field reports
INSERT INTO field_reports (title, content, organization_id, created_by)
SELECT 
    'Field Report ' || generate_series,
    'Test content for report #' || generate_series,
    '11111111-1111-1111-1111-111111111111'::uuid,
    auth.uid()
FROM generate_series(1, 3);

-- 5. Create test submittals
INSERT INTO submittals (title, description, status, organization_id, created_by)
SELECT 
    'Submittal ' || generate_series,
    'Test submittal #' || generate_series,
    'pending',
    '11111111-1111-1111-1111-111111111111'::uuid,
    auth.uid()
FROM generate_series(1, 3);

-- 6. Create test contacts
INSERT INTO contacts (name, email, company, organization_id, created_by)
VALUES 
    ('John Doe', 'john@example.com', 'ABC Construction', '11111111-1111-1111-1111-111111111111'::uuid, auth.uid()),
    ('Jane Smith', 'jane@example.com', 'XYZ Engineering', '11111111-1111-1111-1111-111111111111'::uuid, auth.uid()),
    ('Bob Johnson', 'bob@example.com', 'Test Corp', '11111111-1111-1111-1111-111111111111'::uuid, auth.uid());

-- 7. Verify all data persists - FINAL COUNT
SELECT 
    'Data Persistence Test' as test_name,
    'VBA Projects' as table_name,
    COUNT(*) as record_count
FROM vba_projects
WHERE organization_id = '11111111-1111-1111-1111-111111111111'::uuid
UNION ALL
SELECT 
    'Data Persistence Test',
    'Projects',
    COUNT(*)
FROM projects
WHERE organization_id = '11111111-1111-1111-1111-111111111111'::uuid
UNION ALL
SELECT 
    'Data Persistence Test',
    'Field Reports',
    COUNT(*)
FROM field_reports
WHERE organization_id = '11111111-1111-1111-1111-111111111111'::uuid
UNION ALL
SELECT 
    'Data Persistence Test',
    'Submittals',
    COUNT(*)
FROM submittals
WHERE organization_id = '11111111-1111-1111-1111-111111111111'::uuid
UNION ALL
SELECT 
    'Data Persistence Test',
    'Contacts',
    COUNT(*)
FROM contacts
WHERE organization_id = '11111111-1111-1111-1111-111111111111'::uuid
ORDER BY table_name;

-- 8. Show recent VBA projects to confirm they're saved
SELECT 
    id,
    name,
    project_name,
    project_number,
    status,
    created_at
FROM vba_projects
ORDER BY created_at DESC
LIMIT 5;